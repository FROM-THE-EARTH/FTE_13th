...
SERVO0_RESET = 79 #initial (from 44 to 232)
...
fileName = 'log/csv/testlog_' + nowTime.strftime('%Y-%m%d-%H%M%S') + '.csv'
new_dir_path_img = 'log/image'
...
def setUp():
    ...
    with open(fileName, 'a') as f:
        writer = csv.writer(f)
        writer.writerow(['MilliTime','count','Phase','Acc_x','Acc_y','Acc_z','Gyro_x','Gyro_y','Gyro_z','Mag_x','Mag_y','Mag_z','Temp','Pres','Alti','Lat','Lng','Distance','Azimuth','Angle','Direction'])
    ...
    getThread = threading.Thread(target=camera_thread3, args=()) # 上の関数を実行するスレッドを生成
...
def setData_thread2():
    while True:
        ...
        with open(fileName, 'a', newline="") as f:
            writer = csv.writer(f)
            writer.writerow([currentMilliTime(), round(phase,1),round(phase,1), acc[0], acc[1], acc[2], gyro[0], gyro[1], gyro[2], mag[0], mag[1], mag[2], temp, pres, alti, lat, lng, distance, azimuth, angle, direction])
...
def camera_thread3():
    global count
    #nowTime = datetime.datetime.now()
    imgfileName = 'log/image/capture_' + str(count) + '.jpg'
...
def flying(): #落下検知関数 :飛んでいるときはTrueを返し続ける
    ...
    if subAlti>ALTITUDE_CONST1 and absAlti<ALTITUDE_CONST2 or restTime > 1*10:
        return False
    else:
        True
