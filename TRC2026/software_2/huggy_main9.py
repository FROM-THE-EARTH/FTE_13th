...
ALTITUDE_CONST2 = 5  # m
TARGET_LAT = 38.26089833
TARGET_LNG = 140.85381833
MAG_CONST = 8.53
SERVO0_SET = 900
SERVO0_RESET = 1500
TIRE_CALIB = 30
    ...
    while True:
       ...
       # elif phase == 3.0:
           # print("main():         Phase3 start. GPS navigation")
            ...
            print("main():         Phase3 start. Moving to waypoint 0")
            GPIO.output(LED0, 0)

            target_lat, target_lng = waypoints[0]  # ウェイポイント0の座標を取得
            print(f"phase3startBeforemove_to_waypoint waypoint0 = {waypoints[0]}")
            move_to_waypoint(target_lat, target_lng)  # ウェイポイント0へ移動
            print(f"phase3Aftermove_to_waypoint waypoint0 = {waypoints[0]}")
            current_waypoint_index = 1
            #phase = 4.0  # ウェイポイント0に到達後、フェーズ4へ移行


        elif phase == 4.0:
            print("main():         Phase4 start. waypoint movement")
            GPIO.output(LED0, 1)
            ...
            while current_waypoint_index < len(waypoints):
                ...
                #current_waypoint_index += 1  # インデックスを次に進める
            ...
            #for target_lat, target_lng in waypoints[1:]:
             #   move_to_waypoint(target_lat, target_lng)
              #  if detectCorn():
               #     phase = 5.0
                #    break
        ...
        #elif phase == 5.0:
         #   print("main():         Phase5 start. camera search mode")
          #  while True:
           #     if detectCorn():
            #        if colorCorn:
             #           phase = 6.0
    ...
    try:
        ...
        while True:
            ...
            if direction == 0.0: #phase0
                ...
                myduty = 80
                LF0.ChangeDutyCycle(60) #direction=-180で最大100
                LB0.ChangeDutyCycle(0)
                RF0.ChangeDutyCycle(0)
                ...
                LF0.ChangeDutyCycle(60+direction*30/180) #direction=-180で最大100
                LB0.ChangeDutyCycle(0)
                RF0.ChangeDutyCycle(0)
                RB0.ChangeDutyCycle(40-direction*30/180)
                pi.set_servo_pulsewidth(SERVO0,SERVO0_SET)
...
def calcDistance(_lat, _lng): #距離計算用関数
    global distance


    EARTH_RADIUS = 6378136.59
    #centerLat = (math.pi/180)*(lat+TARGET_LAT)/2 
    centerLat = (math.pi/180)*(lat+_lat)/2  # target_lat を使用
    dx = (math.pi/180)*EARTH_RADIUS*(_lng-lng)*math.cos(centerLat)  # target_lng を使用
    dy = (math.pi/180)*EARTH_RADIUS*(_lat-lat)
    #dx = (math.pi/180)*EARTH_RADIUS*(TARGET_LNG-lng)*math.cos(centerLat)
    #dy = (math.pi/180)*EARTH_RADIUS*(TARGET_LAT-lat)
    distance = math.sqrt(dx*dx+dy*dy)
    
    print(f"dx = {dx}, dy={dy}, dis = {distance}")
    print(f"targetlat = {_lat}, targetlng = {_lng}")
    
    

def calcAngle(_lat, _lng):  # 引数を追加
    global angle
...
def move_to_waypoint(_lat, _lng):
    global distance
    
    while True:
        calcDistance(_lat, _lng)  # 修正後の関数を使用
        if distance < 2:
            break
        calcAngle(_lat, _lng)  # 修正後の関数を使用
        calcAzimuth()
        setDirection()
        time.sleep(0.1)
