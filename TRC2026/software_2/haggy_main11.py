...
ALTITUDE_CONST2 = 5  # m
TARGET_LAT = 38.26081000
TARGET_LNG = 140.85420333
MAG_CONST = 8.53
SERVO0_SET = 900
SERVO0_RESET = 1500
TIRE_CALIB = 30
    ...
    while True:
       ...
       # elif phase == 3.0:
           # print("main():         Phase3 start. GPS navigation")
            ...
            if cameraFlag == True:
                while True:
                    if distance<100: #目的地との距離が10m以下ならばphase4へ
                        phase = 4.0
                        break
            else: #カメラが使えなかった時用
                while True:
    ...
    try:
        ...
        while True:
            ...
            if direction == 0.0: #phase0
                ...
                LB0.ChangeDutyCycle(0)
                RF0.ChangeDutyCycle(0)
                RB0.ChangeDutyCycle(50-direction*30/180)
                pi.set_servo_pulsewidth(SERVO0,SERVO0_SET)
...
def move_to_waypoint(target_lat, target_lng):
    ...
    while True:
        calcDistance(target_lat, target_lng)  # 修正後の関数を使用
        if distance < 2:
            break
        calcAngle(target_lat, target_lng)  # 修正後の関数を使用
        calcAzimuth()
        setDirection()
        time.sleep(0.1)
