...
class corn:
    ...
    def __init__(self,width=320,height=240,camera_for_pi=True):
        ...
        if camera_for_pi:
            ...
            self.camera.awb_gains = (1.5,1.9)  #original:1.5,1,9
            self.camera.brightness = 50  #original:50
    ...
    def find_far_cone(self,preview=False):
        ...
        min = np.array([0,15,110],np.uint8) #0,50,110
        max = np.array([10,180,255],np.uint8)#10,180,255
        mask = cv2.inRange(hls,min,max)
