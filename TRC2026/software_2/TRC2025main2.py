...
ALTITUDE_CONST1 = 32 #m :最大と最小の差
ALTITUDE_CONST2 = 5 #m :最小と現在の差の絶対値
TARGET_LAT = 30.73262500
TARGET_LNG = 130.99394167
MAG_CONST = 8.53 #地磁気補正用の偏角
SERVO0_SET = 1100 #initial (from 44 to 232)
SERVO0_RESET = 1500 #moving
LMotorPW = 50
RMotorPW = 40
...
def main():
    ...
    while True:
        if phase == 0.0:
            ...
            if cameraFlag == True:
                while True:
                    if distance<6: #目的地との距離が10m以下ならばphase4へ
                        phase = 4.0
                        break
    ...
    try:
        ...
        while True:
            ...
            if direction == 0.0: #phase0
                ...
                myduty = 80
                LF0.ChangeDutyCycle(LMotorPW) #direction=-180で最大100
                LB0.ChangeDutyCycle(0)
                RF0.ChangeDutyCycle(0)
                RB0.ChangeDutyCycle(RMotorPW)
            elif direction == -450.0:
                LF0.ChangeDutyCycle(35) #direction=-180で最大100
                LB0.ChangeDutyCycle(0)
                RF0.ChangeDutyCycle(0)
                RB0.ChangeDutyCycle(5)
            elif direction == 450.0:
                LF0.ChangeDutyCycle(5) #direction=-180で最大100
                LB0.ChangeDutyCycle(0)
                RF0.ChangeDutyCycle(0)
                RB0.ChangeDutyCycle(35)
            else:
                LF0.ChangeDutyCycle(LMotorPW+direction*30/180) #direction=-180で最大100
                LB0.ChangeDutyCycle(0)
                RF0.ChangeDutyCycle(0)
                RB0.ChangeDutyCycle(RMotorPW-direction*30/180)
                pi.set_servo_pulsewidth(SERVO0,SERVO0_SET)
